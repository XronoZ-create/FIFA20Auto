"""
Класс-клиент симмуляции геймпада
...
class DS4Device(Device):
	...
	def press(self, key, timeout=0):
		print(key)
		self.send(key=key, value=True)
		time.sleep(0.1)
		self.send(key=key, value=False)
		if timeout != 0:
			time.sleep(timeout)

	def press_ncount(self, key, count, timeout=0.5):
		for self.a in range(0, int(count)):
			self.press(key=key)
			time.sleep(timeout)

	def right_on_stick(self, timeout=0):
		print('right on stick')
		self.send(key='left-stick-X', value=1)
		time.sleep(0.1)
		self.send(key='left-stick-X', value=0)
		if timeout != 0:
			time.sleep(timeout)

	def down_on_stick(self, timeout=0):
		print('down on stick')
		self.send(key='left-stick-Y', value=1)
		time.sleep(0.1)
		self.send(key='left-stick-Y', value=0)
		if timeout != 0:
			time.sleep(timeout)

	def right_on_stick_ncount(self, count, timeout=0.5):
		for self.a in range(0, int(count)):
			self.right_on_stick()
			time.sleep(timeout)

	def down_on_stick_ncount(self, count, timeout=0.5):
		for self.a in range(0, int(count)):
			self.down_on_stick()
			time.sleep(timeout)

	def left_on_stick(self, timeout=0):
		print('left on stick')
		self.send(key='left-stick-X', value=-1)
		time.sleep(0.1)
		self.send(key='left-stick-X', value=0)
	...
	def up_on_stick(self, timeout=0):
		print('up on stick')
		self.send(key='left-stick-Y', value=-1)
		time.sleep(0.1)
		self.send(key='left-stick-Y', value=0)
	...
	def up_on_stick_ncount(self, count, timeout=0.5):
		for self.a in range(0, int(count)):
			self.up_on_stick()
			time.sleep(timeout)

	def left_on_stick_ncount(self, count, timeout=0.5):
		for self.a in range(0, int(count)):
			self.left_on_stick()
			time.sleep(timeout)

	def stick_to_onehourse(self):
		print('up and right on stick and x')
		self.send(key='left-stick-Y', value=-0.7)
		self.send(key='left-stick-X', value=0.7)
		time.sleep(0.2)
		self.press(key='a-button')
		self.send(key='left-stick-Y', value=0)
		self.send(key='left-stick-X', value=0)
		time.sleep(0.2)
	...
	def right_stick_down(self):
		...
		self.send(key='right-stick-Y', value=1)
		time.sleep(0.2)
		self.send(key='right-stick-Y', value=0)
		time.sleep(0.2)

	def down_on_stick_and_x(self):
		print('down on stick and x')
		self.send(key='left-stick-Y', value=1)
		time.sleep(0.2)
		self.press(key='a-button')
		self.send(key='left-stick-Y', value=0)
